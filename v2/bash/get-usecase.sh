...
echo "Retrieving use case $USECASE_ID..."
echo ""
...
set +e
HTTP_CODE=$(curl -S -s -o "$BODY_FILE" -D "$HEADERS_FILE" -w "%{http_code}" -X GET "$BASE_URL_API/usecases/$USECASE_ID" \
